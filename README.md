...
The nonroot variant runs as a non-root user by default, satisfying container security best practices. The explicit `USER nonroot` statement is included for clarity.
...
Variables passed to the module are `env`, `project_id`, `region`, `container_image`, `service_name`, `commit_sha`, `is_public`, and `deletion_protection`. For all deployments `is_public` is `false` and `deletion_protection` is `false`. The service is not publicly accessible by default; access control is managed externally (e.g., via a load balancer or IAP).
...
`promote-container-to-prod-registry` authenticates with the prod GCP project, then pulls the already-tested image from the QA Artifact Registry, retags it for the prod registry, and pushes it. This means the exact same image that passed QA is what gets deployed to production — no rebuild happens.
...
When a release is ready a `release/**` branch is created and a PR is opened against it (or commits are pushed to it). CI runs validate and build and pushes to the QA Artifact Registry. The CD pipeline deploys to `qa`.
...
### 1. Branching Model
The project utilizes an **Environment-Branching** model. It relies on specific branch patterns to trigger environment-specific logic:
...
Data Masking: The JSON logging configuration in `logback-spring.xml` would need filters (e.g., `PatternMaskingLayout`) to ensure PII is never written to stdout.
